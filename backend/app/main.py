...
async def run_pipeline(job_id: str):
    ...
    try:
        ...
        if input_path.endswith(".pdf"):
            ...
            doc.close()
            event("phase1", "info", f"PDF: {n_pages} pages")
            if source_title:
            ...
            first_page_text = document_text[:5000]

        _pdf = input_path if input_path.endswith(".pdf") else None
        detection_result = await try_step(
            "Detecting and summarizing invention (full document)",
            phase="phase1",
            fn=lambda: detect_and_summarize_invention(
                document_text, source_pdf_path=_pdf),
            validator_input=document_text[:20000] if document_text else "(PDF sent natively)",
        )
        ...
        if status_det == "Absent":
            event("phase1", "no_invention",
                  ...
                  {"doc_type": doc_type, "first_page_preview": (first_page_text or document_text)[:1000]})
        ...
        await maybe_review(
            ...
            task_desc="Decide whether the summary is grounded in the document text "
                      "and captures the core technical contribution.",
            original_input=first_page_text[:5000],
            output_text=summary,
        )
        ...
        if status_det == "Implied":
            ...
            from app.email_notify import DEFAULT_NOTIFY, send_report
            notify_to = job.get("notify_email") or DEFAULT_NOTIFY
            if notify_to:
                subject = f"Patent Analysis Complete (Implied): {source_title or job.get('filename', 'report')}"
                err = send_report(notify_to, subject, md, job_id=job_id)
                if err:
                    event("phase5", "email_failed", f"Email to {notify_to}: {err}")
                else:
                    event("phase5", "email_sent", f"Report emailed to {notify_to}")
            job["status"] = "completed"
            _save_job(job)
            return
        ...
        event("phase3b", "start", f"Reranking {len(all_docs)} documents by per-axis embedding")
        try:
            from patent_analyzer.semantic_search import rerank_by_axes, rerank_by_embedding
            ...
            for item in checklist:
                if isinstance(item, dict):
                    ...
                    if criterion:
                        axis_groups.setdefault(axis, []).append(criterion)

            axes_queries = []
            for axis_name, criteria in axis_groups.items():
                query = f"{axis_name}: " + " ".join(criteria)
                axes_queries.append({"axis": axis_name, "query": query})

            if axes_queries:
                per_axis = max(4, 30 // len(axes_queries) + 2)
                ranked, axis_coverage = rerank_by_axes(
                    axes_queries, all_docs,
                    fallback_query=summary,
                    per_axis=per_axis, total_limit=30)
                cov_str = ", ".join(f"{k}: {v}" for k, v in axis_coverage.items())
                event("phase3b", "info",
                      f"Per-axis rerank: {len(axes_queries)} axes, "
                      f"{len(ranked)} unique candidates. Coverage: {cov_str}")
            else:
                ranked = rerank_by_embedding(summary, all_docs, limit=30)
                event("phase3b", "info", "Fallback: top 30 by overall embedding (no axis info)")
        except Exception as e:
        ...
        notify_to = job.get("notify_email") or DEFAULT_NOTIFY
        if notify_to:
            subject = f"Patent Analysis Complete: {source_title or job.get('filename', 'report')}"
            err = send_report(notify_to, subject, md, job_id=job_id)
            if err:
                event("phase5", "email_failed", f"Email to {notify_to}: {err}")
            else:
