...
def rerank_by_axes(
    ...
    """Per-axis reranking: each innovation axis independently selects top candidates.

    axes_queries: [{"axis": "name", "query": "embedding text"}, ...]
    Returns (ranked_docs, coverage) where coverage maps axis name to pick count.
    """
    if not documents or not axes_queries:
        return rerank_by_embedding(fallback_query, documents, limit=total_limit), {}

    doc_texts = _build_doc_texts(documents)
    if not doc_texts:
        return [], {}

    print(f"Per-axis rerank: {len(axes_queries)} axes × {len(doc_texts)} docs")
    doc_vecs = embed_texts(doc_texts)

    selected: dict[int, dict] = {}  # idx -> {"score": best_sim, "axes": [axis_names]}
    coverage: dict[str, int] = {}

    for ax in axes_queries:
        ...
        for idx in ranked:
            ...
            sim = float(sims[idx])
            if idx in selected:
                selected[idx]["axes"].append(axis_name)
                if sim > selected[idx]["score"]:
                    selected[idx]["score"] = sim
            else:
                selected[idx] = {"score": sim, "axes": [axis_name]}
            picked += 1
        coverage[axis_name] = picked

    if len(selected) < total_limit:
        fallback_vec = embed_texts([fallback_query])[0]
        fallback_sims = cosine_similarity(fallback_vec, doc_vecs)
        for idx in np.argsort(fallback_sims)[::-1]:
            if len(selected) >= total_limit:
                break
            idx = int(idx)
            if idx not in selected:
                selected[idx] = {"score": float(fallback_sims[idx]), "axes": ["_fallback"]}

    results = []
    for idx, info in sorted(selected.items(), key=lambda x: x[1]["score"], reverse=True):
        doc = documents[idx].copy()
        doc["semantic_score"] = info["score"]
        doc["_matched_axes"] = info["axes"]
        results.append(doc)

    return results[:total_limit], coverage
